...
/* Sam: toegevoegd, want ontbrak. in de officiele code staat het wel */
/**
...
function exhibit_builder_user_can_delete($exhibit = null, $user = null)
{
    ...
    if (version_compare(OMEKA_VERSION, '2.0-dev', '>=')) {
        ...
        $canDeleteOthers = $acl->isAllowed($user, 'ExhibitBuilder_Exhibits', 'deleteAll');
    } else {
        $canDeleteSelf = $acl->checkUserPermission('ExhibitBuilder_Exhibits', 'deleteSelf');
        $canDeleteOthers = $acl->checkUserPermission('ExhibitBuilder_Exhibits', 'deleteAll');
    }
...
/**
 * Returns an array of thumbs for thumbnail select field in the exhibit metadata form
 *
 * @param Exhibit
 * @return array of thumbnails made out of items of the pages belonging to the exhibit
 **/
function exhibit_builder_get_thumbs_array($exhibit)
{
	$array;
	$items = get_items(array('exhibit' => $exhibit));
	set_items_for_loop($items);
	if (has_items_for_loop()):

	while (loop_items()):
	$item = get_current_item();
	if(item_has_thumbnail() || digitool_item_has_digitool_url($item)){
		$array[$item->id] = item('Dublin Core', 'Title');
	}
