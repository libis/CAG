        ...
        <fieldset>
            ...
            <?php 
            	if($exhibit->thumbnail){
            		echo "Current thumbnail: ".$exhibit->thumbnail."<br>Choose another thumbnail  ";
            	}
            ?>
			<select name="thumbnail">
			 <!--<?php
            	//if($exhibit->Sections){ 
            	//echo select(array('name'=>'thumbnail', 'id'=>'thumbnail', 'class'=>'textinput'),
            	 //	exhibit_builder_get_thumbs_array($exhibit),$exhibit->thumbnail,"Thumbnail (belonging to an item of a page in this exhibit)");
            	//}else{
            		//echo"<label>Thumbnail</label>Only available after creating a page containing an item with a thumbnail";
            	//}?>-->
				<?php $folder=dir("../themes/CAG/images/verhalen_thumbs");
     
				while($folderEntry=$folder->read())
				{
					if($folderEntry == $exhibit->thumbnail)
						echo "<option selected='selected' value='".$folderEntry."'>".$folderEntry."</option>";
					else
						echo "<option value='".$folderEntry."'>".$folderEntry."</option>";
				}
